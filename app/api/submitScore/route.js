...
export async function POST(request) {
  try {
    ...
    let updateField = game.player1Address === playerAddress ? 'player1Score' : 'player2Score';
    ...
    crossOriginIsolated.log("Result", result)
     ...
     if (updatedGame.player1Score !== null && updatedGame.player2Score !== null) {
       const winner = updatedGame.player1Score > updatedGame.player2Score ? updatedGame.player1Address : updatedGame.player2Address;
       ...
       try {
        ...
        return NextResponse.json({ message: "Score submitted and game settled successfully" });
      } catch (error) {
        console.error("Error settling the game:", error);
        return NextResponse.json({ message: "Score submitted but failed to settle the game", error: error.message }, { status: 500 });
      }
     }     
 
     return NextResponse.json({ message: "Score submitted successfully" });
   } catch (error) {
     return NextResponse.json({ message: "Error submitting score", error: error.message }, { status: 500 });
   }
 }